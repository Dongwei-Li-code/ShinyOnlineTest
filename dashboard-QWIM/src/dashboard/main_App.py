...
from src.dashboard.modules.JM_portfolios_module import JM_portfolios_server, JM_portfolios_ui
from src.dashboard.modules.GMM_portfolios_module import GMM_portfolios_server, GMM_portfolios_ui
from src.dashboard.modules.HMM_portfolios_module import HMM_portfolios_server, HMM_portfolios_ui
...
app_UI = ui.page_navbar(
    ui.nav_panel(
        "Market Regimes and Wealth Management Strategies", 
        ui.layout_sidebar(
            ui.sidebar(
                shinyswatch.theme_picker_ui("cosmo"),
                ui.p("Regime-Based Strategies and Visualization"),
                ui.hr(),
                ui.tags.h4("About"),
                ui.p("This dashboard visualizes and analyzes both the financial asset data and the regime-based strategies built upon it."),
                ui.p("Use the tabs below to explore the input financial asset data and regime-based strategies."),
                ui.p("The JM_portfolios, GMM_portfolios, and HMM_portfolios represent regime-based strategies built on Statistical Jump Model, Gaussian Mixture Model, and Hidden Markov Model respectively."),
                ui.hr(),
                ui.tags.h4("Navigation"),
                ui.p("1. Input Financial Asset Data: Select input financial asset data and filter data (at most 5 assets)"),
                ui.p("2. Input Data Analysis: Visualize and analyze selected input time series"),
                ui.p("3. Input Data Insights: View results and insights of the input financial asset data"),
                ui.p("4. JM_Portfolios: View the regime-based strategies built upon Statistical Jump Model"),
                ui.p("5. GMM_Portfolios: View the regime-based strategies built upon Gaussian Mixture Model"),
                ui.p("6. HMM_Portfolios: View the regime-based strategies built upon Hidden Markov Model"),
                ui.hr(),
                ui.a("GitHub", href="https://github.com/Dongwei-Li-code/QWIM2025MarketRegimes", target="_blank"),
                ui.hr(),
                ui.p(f"Version: {__version__}"),
                ui.p(f"Build: {__build_date__}"),
            ),
            ui.navset_tab(
                ui.nav_panel("Input Financial Asset Data", inputs_ui("ID_inputs")),
                ui.nav_panel("Input Data Analysis", analysis_ui("ID_analysis")),
                ui.nav_panel("Input Data Insights", results_ui("ID_results")),
                # ui.nav_panel("Portfolios", portfolios_ui("ID_portfolios")),
                ui.nav_panel("JM_Portfolios", JM_portfolios_ui("ID_JM_portfolios")),
                ui.nav_panel("GMM_Portfolios", GMM_portfolios_ui("ID_GMM_portfolios")),
                ui.nav_panel("HMM_Portfolios", HMM_portfolios_ui("ID_HMM_portfolios")),
                id="main_tabs",
            ),
        ),
    ),
...
def app_Server(input, output, session):
    ...
    try:
        logger.info("Initializing JM portfolios module...")
        # Use only the parameters that the function actually accepts
        JM_portfolios_server(
            "ID_JM_portfolios", 
            inputs_data=inputs_data, 
            data_r=data_r
        )
        logger.info("JM Portfolios module initialized successfully")
    except Exception as e:
        logger.error(f"Error initializing JM portfolios module: {str(e)}")
        import traceback
        logger.error(traceback.format_exc())

    try:
        logger.info("Initializing GMM portfolios module...")
        # Use only the parameters that the function actually accepts
        GMM_portfolios_server(
            "ID_GMM_portfolios", 
            inputs_data=inputs_data, 
            data_r=data_r
        )
        logger.info("GMM Portfolios module initialized successfully")
    except Exception as e:
        logger.error(f"Error initializing GMM portfolios module: {str(e)}")
        import traceback
        logger.error(traceback.format_exc())

    try:
        logger.info("Initializing HMM portfolios module...")
        # Use only the parameters that the function actually accepts
        HMM_portfolios_server(
            "ID_HMM_portfolios", 
            inputs_data=inputs_data, 
            data_r=data_r
        )
        logger.info("HMM Portfolios module initialized successfully")
    except Exception as e:
        logger.error(f"Error initializing HMM portfolios module: {str(e)}")
        import traceback
        logger.error(traceback.format_exc())
