...
PORTFOLIO_VALUES_FILE = PROCESSED_DATA_DIR / "GMM_portfolio_values.csv"
"""Path to the portfolio values CSV file."""

BENCHMARK_VALUES_FILE = PROCESSED_DATA_DIR / "GMM_benchmark_portfolio_values.csv"
"""Path to the benchmark values CSV file."""

PORTFOLIO_WEIGHTS_FILE = RAW_DATA_DIR / "GMM_portfolio_weights_ETFs.csv"
...
@module.server
def GMM_portfolios_server(input, output, session, inputs_data=None, data_r=None):
    ...
    def load_regime_data():
        try:
            path = PROCESSED_DATA_DIR / "GMM_regime_data.csv"
            df = pd.read_csv(path)
    ...
    def output_ID_regime_summary_ui():
        try:
            ...
            grouped.index = grouped.index.map({0: "Regime_0", 1: "Regime_1"})

            summary = grouped.T
            summary = summary[["Regime_0", "Regime_1"]] if "Regime_0" in summary.columns and "Regime_1" in summary.columns else summary
