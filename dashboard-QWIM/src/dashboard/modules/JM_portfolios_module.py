...
PORTFOLIO_VALUES_FILE = PROCESSED_DATA_DIR / "JM_portfolio_values.csv"
"""Path to the portfolio values CSV file."""

BENCHMARK_VALUES_FILE = PROCESSED_DATA_DIR / "JM_benchmark_portfolio_values.csv"
"""Path to the benchmark values CSV file."""

PORTFOLIO_WEIGHTS_FILE = RAW_DATA_DIR / "JM_portfolio_weights_ETFs.csv"
    ...
    * Option toggles for customizing visualizations
    ...
    return ui.page_fluid(
        ...
        ui.navset_card_tab(
            ...
            ui.nav_panel(
                "Regime Explaination and Analysis",
                ui.layout_sidebar(
                    ui.sidebar(
                        ui.h4("Date Range"),
                        ui.input_date_range(
                            "ID_regime_date_range",
                            "Select Date Range:",
                            start=datetime(2018, 1, 1),
                            end=datetime.now(),
                            min=datetime(2010, 1, 1),
                        ...
                        ui.h4("Regime Explanations"),
                            ui.input_checkbox(
                            "ID_regime_show_details",
                            "Show detailed explanations",
                            True
                        ),
                    ),
                    ui.h3("Regime Analysis"),
                    output_widget("output_ID_regime_plot"),
                    ui.hr(),
                    ui.h3("Return Summary by Regime"),
                    ui.output_ui("output_ID_regime_summary_ui"),
                    ui.hr(),
                    ui.h3("Regime Explanation"),
                    ui.output_ui("output_ID_regime_explanation_ui")
                )
            )
...
@module.server
def JM_portfolios_server(input, output, session, inputs_data=None, data_r=None):
        ...
        try:
            ...
            for metric in all_metrics:
                ...
                if isinstance(portfolio_metrics, pd.Series) and metric in portfolio_metrics.index:
                    ...
                    row["My-Portfolio"] = portfolio_metrics.loc[metric, "Strategy"] if len(portfolio_metrics.columns) > 0 else None # row["My-Portfolio"] = portfolio_metrics.loc[metric].iloc[0] if len(portfolio_metrics.columns) > 0 else None
                else:
                ...
                if isinstance(benchmark_metrics, pd.Series) and metric in benchmark_metrics.index:
                    ...
                    row["Benchmark-Portfolio"] = benchmark_metrics.loc[metric, "Strategy"] if len(benchmark_metrics.columns) > 0 else None # row["Benchmark-Portfolio"] = benchmark_metrics.loc[metric].iloc[0] if len(benchmark_metrics.columns) > 0 else None
                else:
    ...
    def load_regime_data():
        try:
            path = PROCESSED_DATA_DIR / "JM_regime_data.csv"
            df = pd.read_csv(path)
    ...
    @output
    @render_widget
    def output_ID_regime_plot():
        try:
            df = load_regime_data()
            if df.empty or "return" not in df.columns or "regime" not in df.columns:
                return px.scatter(title="Invalid regime data format")

            df = df.copy()
            df["Date"] = pd.to_datetime(df["Date"])
            df = df.sort_values("Date")

            date_range = input.ID_regime_date_range()
            ...
            if df.empty:
                return px.scatter(title="No data in selected date range.")

            selected_metric = input.ID_regime_metric()
            if selected_metric == "return":
                df["metric_y"] = df["return"]
                y_label = "Daily Return"
            else:
                df["metric_y"] = np.exp(df["return"].cumsum())
            ...
            for date, row in df.iterrows():
                ...
                if current_regime != prev_regime:
                    shapes.append(dict(
                        ...
                        y0=0,
                        y1=1,
                        fillcolor=regime_color_map.get(prev_regime, "gray"),
                        opacity=1,
                        layer="below",
                        line_width=0
                    ))
            ...
            fig.add_trace(go.Scatter(
                ...
                mode="lines",
                name="Bull Regime",
                line=dict(color="rgba(0, 200, 0, 0.6)", width=10)
            ))
            fig.add_trace(go.Scatter(
                x=[None], y=[None],
                mode="lines",
                name="Bear Regime",
                line=dict(color="rgba(200, 0, 0, 0.6)", width=10)
            ))

            fig.update_layout(
                title="Return Series with Regime Highlighting",
                xaxis_title="Date",
                yaxis_title=y_label,
                shapes=shapes,
                hovermode="x unified",
            ...
            print(f"Error rendering regime plot: {e}")
    ...
    @output
    @render.ui
    def output_ID_regime_summary_ui():
        try:
            df = load_regime_data()
            if df.empty or "return" not in df.columns or "regime" not in df.columns:
                return ui.HTML("<p style='color:red;'>Invalid regime data format</p>")
            ...
            if df.empty:
                return ui.HTML("<p>No data in selected range.</p>")

            grouped = df.groupby("regime")["return"].agg([
                ("Mean Return", "mean"),
                ("Std Dev", "std"),
                ("Min", "min"),
                ("Max", "max"),
            ...
            grouped.index = grouped.index.map({0: "Bull", 1: "Bear"})

            summary = grouped.T
            summary = summary[["Bull", "Bear"]] if "Bull" in summary.columns and "Bear" in summary.columns else summary
            ...
            from great_tables import GT
            if not df.empty:
                subtitle = f"From {df['Date'].min().date()} to {df['Date'].max().date()}"
            else:
                subtitle = "No data in selected date range"
            gt = GT(summary).tab_header("Return Summary by Regime", subtitle=subtitle)
            html_content = gt.as_raw_html()

            return ui.HTML(html_content)

        except Exception as e:
            return ui.HTML(f"<p style='color:red;'>Error: {str(e)}</p>")
        
    
    @output
    @render.ui
    def output_ID_regime_explanation_ui():
        import pandas as pd
        from datetime import datetime

        # 1) Define all bear‐regime periods with both short & long English explanations
        bear_explanations = {
            (datetime(2011, 8, 2),   datetime(2011, 12, 29)): {
                "short": "U.S. Debt Ceiling Crisis & Rating Downgrade",
                "long":  (
                    "This bear market regime lasted from 2011‑08‑02 to 2011‑12‑29. "
                    "It was driven by the U.S. Debt Ceiling Crisis and S&P’s downgrade of U.S. "
                    "sovereign debt from AAA to AA+. These events triggered widespread investor fear, "
                    "causing a prolonged downturn."
                )
            },
            (datetime(2015, 8, 21),  datetime(2015, 10, 6)): {
                "short": "China RMB Devaluation & Global Market Shock",
                "long":  (
                    "This bear market lasted from 2015‑08‑21 to 2015‑10‑06. "
                    "On August 11, 2015, the People’s Bank of China unexpectedly devalued the RMB, "
                    "sparking fears of a slowdown in China’s growth. Equity markets plunged globally, "
                    "spreading the sell‑off."
                )
            },
            (datetime(2015, 12, 11), datetime(2016, 2, 17)): {
                "short": "Fed Rate Hike & Recession Fears",
                "long":  (
                    "This bear regime ran from 2015‑12‑11 to 2016‑02‑17. "
                    "It was sparked by the Fed’s first rate hike since 2006 on December 16, 2015, "
                    "combined with weak economic data in January 2016. Markets priced in recession risk, "
                    "deepening the downturn."
                )
            },
            (datetime(2018, 2, 5),   datetime(2018, 4, 16)): {
                "short": "U.S.–China Trade War Begins",
                "long":  (
                    "This bear market lasted from 2018‑02‑05 to 2018‑04‑16. "
                    "Tariff threats between the U.S. and China fueled uncertainty, "
                    "as investors feared negative economic fallout from escalating trade tensions."
                )
            },
            (datetime(2018, 10, 10), datetime(2019, 1, 14)): {
                "short": "U.S.–China Trade War Escalation",
                "long":  (
                    "This bear regime ran from 2018‑10‑10 to 2019‑01‑14. "
                    "Renewed tariff announcements and negotiation breakdowns reignited fears "
                    "of a protracted trade war, causing another market correction."
                )
            },
            (datetime(2019, 8, 5),   datetime(2019, 9, 3)): {
                "short": "Yield Curve Inversion Signal",
                "long":  (
                    "This bear market lasted from 2019‑08‑05 to 2019‑09‑03. "
                    "A U.S. Treasury yield curve inversion—a classic recession indicator—"
                    "spooked investors, who rotated into safe‑haven assets."
                )
            },
            (datetime(2020, 2, 24),  datetime(2020, 7, 15)): {
                "short": "COVID‑19 Pandemic Shock",
                "long":  (
                    "This bear regime ran from 2020‑02‑24 to 2020‑07‑15. "
                    "Global lockdowns and economic disruptions from the COVID‑19 outbreak "
                    "led to a sharp market collapse as uncertainty peaked."
                )
            },
            (datetime(2020, 9, 3),   datetime(2020, 11, 9)): {
                "short": "Post‑Recovery Technical Correction",
                "long":  (
                    "This bear market lasted from 2020‑09‑03 to 2020‑11‑09. "
                    "After the initial COVID‑19 rebound, stretched valuations—especially in tech—"
                    "triggered a technical pullback amid renewed pandemic worries."
                )
            },
            (datetime(2022, 1, 24),   datetime(2022, 12, 31)): {
                "short": "High Inflation & FED's Quantitative Tightening",
                "long":  (
                    "This bear market lasted from 2022‑01‑24 to 2022‑12‑31. "
                    "U.S. inflation hit a forty‑year peak in early 2022, prompting the Fed to hike rates 11 times from March and restart quantitative tightening in June."
                    "This drained liquidity, sent bond yields soaring, and weighed on risk‑asset valuations."
                )
            },
        }

        # 2) Read the user’s selected date range and detail‐toggle
        start, end = input.ID_regime_date_range()
        show_details = input.ID_regime_show_details()
        sd = pd.to_datetime(start) if start else None
        ed = pd.to_datetime(end)   if end   else None

        # 3) Filter & clip to the chosen window (or include all if none chosen)
        periods = []
        for (s, e), txt in bear_explanations.items():
            if sd and ed and (e < sd or s > ed):
                continue
            cs = max(s, sd) if sd else s
            ce = min(e, ed) if ed else e
            periods.append((cs, ce, txt["short"], txt["long"]))

        if not periods:
            return ui.HTML("<p>No bear‐regime explanations apply to your selected range.</p>")

        # 4) Build the HTML blocks with <details open> when show_details=True
        blocks = []
        for cs, ce, short, long in periods:
            open_attr = " open" if show_details else ""
            blocks.append(ui.HTML(f"""
                <details{open_attr} style="margin-bottom:1em;">
                  <summary>
                    <strong>{cs.date()} – {ce.date()}:</strong> {short}
                  </summary>
                  <div style="padding:0.5em 1em; background:#f9f9f9; border:1px solid #ddd; margin-top:0.5em;">
                    {long}
                  </div>
                </details>
            """))

        # 5) Return a wrapper div
