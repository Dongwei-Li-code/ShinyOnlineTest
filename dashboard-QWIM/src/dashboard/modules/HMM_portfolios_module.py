...
PORTFOLIO_VALUES_FILE = PROCESSED_DATA_DIR / "HMM_portfolio_values.csv"
"""Path to the portfolio values CSV file."""

BENCHMARK_VALUES_FILE = PROCESSED_DATA_DIR / "HMM_benchmark_portfolio_values.csv"
"""Path to the benchmark values CSV file."""

PORTFOLIO_WEIGHTS_FILE = RAW_DATA_DIR / "HMM_portfolio_weights_ETFs.csv"
        ...
        ui.navset_card_tab(
            ...
            ui.nav_panel(
                ...
                ui.layout_sidebar(
                    ...
                    ui.hr(),
                    ui.h4("Return Summary by Regime"),
...
@module.server
def HMM_portfolios_server(input, output, session, inputs_data=None, data_r=None):
    ...
    def load_regime_data():
        _ = input.ID_regime_date_range()
        try:
            path = PROCESSED_DATA_DIR / "HMM_regime_data.csv"
            df = pd.read_csv(path)
            df["Date"] = pd.to_datetime(df["Date"])

            # 替换 regime 编号：0→3, 1→1, 2→0, 3→2
            if "regime" in df.columns:
                df["regime"] = df["regime"].replace({0: 3, 1: 1, 2: 0, 3: 2})
    ...
    def output_ID_regime_plot():
        try:
            ...
            if selected_metric == "return":
                ...
                df["metric_y"] = (1 + df["return"]).cumprod()
            ...
            regime_color_map = {
                0: "rgba(0, 200, 0, 0.2)",
                1: "rgba(150, 200, 0, 0.2)",
                2: "rgba(255, 150, 0, 0.2)",
                3: "rgba(200, 0, 0, 0.2)"
            }
            ...
            regime_legend_colors = {
                0: "rgba(0, 200, 0, 0.6)",
                1: "rgba(150, 200, 0, 0.6)",
                2: "rgba(255, 150, 0, 0.6)",
                3: "rgba(200, 0, 0, 0.6)"
            }
            ...
            for i in range(4):
                fig.add_trace(go.Scatter(
                    ...
                    mode="lines",
                    name=f"Regime {i}",
                    line=dict(color=regime_legend_colors[i], width=10)
                ))
    ...
    def output_ID_regime_summary_ui():
        try:
            ...
            # ✅ 即使为空也保留表结构
            if df.empty:
                df = pd.DataFrame(columns=["Date", "return", "regime"])

            regime_names = [f"regime_{i}" for i in range(4)]
            regime_map = {i: f"regime_{i}" for i in range(4)}
            ...
            grouped = grouped.reindex(regime_names)

            summary = grouped.T.round(5)
